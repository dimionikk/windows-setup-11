...
# Setup.ps1 has its own param() block sharing names with ours (-NoElevate, -Steps);
# dot-sourcing it here runs that block in this same scope and would silently reset
# our variables to their defaults unless we pass our current values back in.
. (Join-Path $PSScriptRoot 'Setup.ps1') -NoElevate:$NoElevate -Steps $Steps
...
function Invoke-SystemSnapshot {
    ...
    # $Id порожній (або $null) - крок вважається завжди вибраним (для підсумкових файлів:
    # секрети, README, ЩО-ВСТАНОВИТИ, .zip), інакше - звіряється зі списком -Steps.
    function Step {
...
"@ | Set-Content (F 'system-info.txt') -Encoding UTF8
    ...
    Step 'installed_programs' "Встановлені програми (реєстр)" {
        Get-ItemProperty @(
    ...
    Step 'winget_export' "winget export" {
    ...
    Step 'env_vars' "Змінні середовища та PATH (User + Machine)" {
    ...
    Step 'autostart' "Автозапуск (реєстр + папки Startup)" {
    ...
    Step 'hosts' "hosts-файл" {
        CopyIf "$env:WINDIR\System32\drivers\etc\hosts" (F 'hosts.txt')
    }

    Step 'config_files' "Конфіги (git / SSH / PowerShell / Windows Terminal / VS Code)" {
        @(
    ...
    if ($isAdmin) {
        Step 'win_features_enabled' "Компоненти Windows (увімкнені)" {
        ...
        Step 'win_features_fod' "Features on Demand" {
        ...
        Step 'drivers_thirdparty' "Сторонні драйвери" {
    ...
    Step '' "Сканування зібраного на секрети" {
        ...
        $exts = @('.txt','.csv','.json','.xml','.ini','.ps1','.config','.gitconfig')
        ...
        $body.Add("профіль PowerShell, .gitconfig, settings.json розширень VS Code,")
...
Чутливе:
  ...
  - config-files\ - .gitconfig, профіль PowerShell, settings.json (VS Code / Terminal), .ssh\config
  - hosts.txt, system-info.txt
...
5. Решта програм - ЩО-ВСТАНОВИТИ.txt (або кнопка "Встановити зі знімка" в меню)
6. config-files\ назад: .gitconfig, профіль PowerShell, Terminal / VS Code settings.json, .ssh\config
7. VS Code - Settings Sync або vscode-extensions.txt
8. Звірка - installed-programs.csv
...
| config-files\ | git, PowerShell, Terminal, VS Code, .ssh\config |
...
креди .aws/.azure/.docker. Нішеві категорії (тема, реєстрові твіки, принтери,
VPN, Wi-Fi, Steam тощо) свідомо не збираються - переналаштуй вручну за потреби.
"@ | Set-Content (F 'README.md') -Encoding UTF8
    ...
    Step '' "Генерую ЩО-ВСТАНОВИТИ.txt" {
        ...
        "Конфіги з config-files назад","Компоненти Windows - windows-features-enabled.csv",
        "Wi-Fi, принтери, VPN, тема - переналаштувати вручну" | ForEach-Object { $out.Add("[ ] $_") }
        $out.Add("")
        $out.Add('='*68)
    ...
    if ($script:secretCount -gt 0 -and -not $Redact) {
        Write-Host "`n  УВАГА: $($script:secretCount) місць схожих на ключі/паролі - див. !МОЖЛИВІ-СЕКРЕТИ.txt" -ForegroundColor Red
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    Invoke-SystemSnapshot -OutputRoot $OutputRoot -Zip:$Zip -Redact:$Redact -NoPrompt:$NoPrompt -Steps $Steps

    if ($MyInvocation.MyCommand.Path -and -not $NoElevate) {
